...
- 【必须】API网关只接受Restful类型的API，暂时不接受其他类型，所以下面的API只是Restful接口规范
- 【必须】API返回值只接受json类型的返回值
- 【建议】**由于厂商已有大量接口，接口改造难度比较大，所以建议提供的新接口按下面文档提供，老接口可以酌情改造**

#### API版本

所有的 `API` 建议保持向后兼容，你在引入新版本 `API` 的同时确保旧版本 `API` 仍然可用。所以 `应该` 为其提供版本支持。
...
- URL 的命名建议全部小写
- URL 中资源（`resource`）的命名 `建议` 是名词且是复数形式
- URL `建议` 是易读的
- URL `一定不可` 暴露服务器架构

> 至于 URL 是否建议使用连字符（`-`） 或下划线（`_`），不做硬性规定，请酌情处理
...
- [https://api.example.com/zoos/{zoo}/animals](https://api.example.com/zoos/%7Bzoo%7D/animals)
...
1. 删除资源 `建议` 用 `DELETE` 方法
2. 创建新的资源 `建议` 使用 `POST` 方法
3. 更新资源 `建议` 使用 `PUT` 方法
4. 获取资源信息 `建议` 使用 `GET` 方法
...
所有 `URL` 参数 `建议` 是全小写，可以使用下划线类型的参数形式。

> 分页参数 `建议` 固定为 `page`、`per_page`
...
所有的 `API` 响应，`建议` 遵守 `HTTP` 设计规范，`建议` 选择合适的 `HTTP` 状态码。`一定不可` 所有接口都返回状态码为 `200` 的 `HTTP` 响应，如：
...
> 只有来自客户端的请求被正确的处理后才能返回 `2xx` 的响应，所以当 API 返回 `2xx` 类型的状态码时，前端 `建议` 认定该请求已处理成功。

建议强调的是，所有 `API` `一定不可` 返回 `1xx` 类型的状态码。当 `API` 发生错误时，`建议` 返回出错时的详细信息。 考虑到易读性和客户端的易处理性，我们 `建议` 把错误信息直接放到响应实体中，并且错误格式 `应该` 满足如下格式：
...
其中错误码（`error_code`）`建议` 和 `HTTP` 状态码对应，也方便错误码归类，如：
...
`200` 状态码是最常见的 `HTTP` 状态码，在所有 **成功** 的 `GET` 请求中，`建议` 返回此状态码。`HTTP` 响应实体部分 `建议` 直接就是数据，不要做多余的包装。
...
- 在使用 `DELETE` 方法删除资源 **成功** 时，`建议` 返回该状态码
...
> 当服务器无法从其他 4xx 类型的状态码中找出合适的来表示错误类型时，都 `建议` 返回该状态码。
...
该状态码表示当前请求需要身份认证，以下情况都 `建议` 返回该状态码。
...
都 `建议` 返回该状态码，若该资源已永久不存在，则 `应该` 返回 `410` 响应。

#### 405 Method Not Allowed

当客户端使用的 `HTTP` 请求方法不被服务器允许时，`建议` 返回该状态码。

> 如客户端调用了 `POST` 方法来访问只支持 GET 方法的 API

该响应 `建议` 返回一个 `Allow` 头信息用以表示出当前资源能够接受的请求方法的列表。
...
客户端请求超时时 `建议` 返回该状态码，需要注意的时，该状态码表示 **客户端请求超时**，在涉及第三方 `API` 调用超时时，`一定不可` 返回该状态码。

#### 409 Confilct

该状态码表示因为请求存在冲突无法处理。如通过手机号码提供注册功能的 `API`，当用户提交的手机号已存在时，`建议` 返回此状态码。
...
`建议` 为所有的 API 设置 Rate Limit 支持。

#### 500 Internal Server Error

该状态码 `建议` 在服务器出错时抛出，对于所有的 `500` 错误，都 `应该` 提供完整的错误信息支持，也方便跟踪调试。
